...
const scroll = () => {
  const sections = Array.from(
    document.querySelectorAll(".sticky-scroll-swap_section")
  );

  sections.forEach((section) => {
    const mediaItems = Array.from(
      section.querySelectorAll(".sticky-scroll-swap_media-item")
    );
    const contentItems = Array.from(
      section.querySelectorAll(".sticky-scroll-swap_content-item")
    );

    function activateItem(index) {
      mediaItems.forEach((item) => gsap.to(item, { autoAlpha: 0 }));
      contentItems.forEach((item) =>
        gsap.to(item, { autoAlpha: INITIAL_CONTENT_OPACITY })
      );
